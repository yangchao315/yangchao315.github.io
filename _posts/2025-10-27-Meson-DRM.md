...
- [模块概述](#模块概述)
- [数据流架构图](#数据流架构图)
- [Meson Driver (meson\_drv.c)](#meson-driver-meson_drvc)
- [CRTC 模块 (meson\_crtc.c)](#crtc-模块-meson_crtcc)
	- [关键数据结构](#关键数据结构)
	- [drm\_crtc\_funcs 实现](#drm_crtc_funcs-实现)
	- [drm\_crtc\_helper\_funcs 实现](#drm_crtc_helper_funcs-实现)
	- [VSync 中断处理](#vsync-中断处理)
- [Plane 模块](#plane-模块)
	- [Primary Plane (meson\_plane.c)](#primary-plane-meson_planec)
	- [Overlay Plane (meson\_overlay.c)](#overlay-plane-meson_overlayc)
	- [Plane 与 CRTC 的绑定](#plane-与-crtc-的绑定)
- [Encoder 模块 (meson\_encoder\_hdmi.c)](#encoder-模块-meson_encoder_hdmic)
	- [Bridge 模式架构](#bridge-模式架构)
	- [drm\_bridge\_funcs 实现](#drm_bridge_funcs-实现)
	- [HDMI Pipeline 初始化](#hdmi-pipeline-初始化)
- [VPU 子系统模块](#vpu-子系统模块)
	- [VIU - Video Input Unit (meson\_viu.c)](#viu---video-input-unit-meson_viuc)
	- [VPP - Video Post Processing (meson\_vpp.c)](#vpp---video-post-processing-meson_vppc)
	- [VENC - Video Encoder (meson\_venc.c)](#venc---video-encoder-meson_vencc)
	- [VCLK - Video Clock (meson\_vclk.c)](#vclk---video-clock-meson_vclkc)
- [AFBC 模块 (meson\_osd\_afbcd.c)](#afbc-模块-meson_osd_afbcdc)
	- [GXM vs G12A AFBC 差异](#gxm-vs-g12a-afbc-差异)
	- [AFBC 与 Plane 的协作](#afbc-与-plane-的协作)
- [RDMA 模块 (meson\_rdma.c)](#rdma-模块-meson_rdmac)
- [模块串联关系](#模块串联关系)
- [SoC 兼容性](#soc-兼容性)
- [参考文档](#参考文档)
